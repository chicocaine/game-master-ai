...
from core.enums import DifficultyType, RestType
...
class Encounter:
    ...
    cleared: bool
    clear_reward: int
    enemies: List[Enemy] = field(default_factory=list) # stored in JSON as a list of `enemy_id` (data_id, only given an instance_id after instantiation)
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "enemies": [enemy.id for enemy in self.enemies],
        }
...
class Room:
    ...
    name: str
    description: str
    is_visited: bool # default false, true when first visited
    is_cleared: bool # when all encounters are cleared, default true if encounters are empty
    is_rested: bool # defuult false, true when the party decides to rest
    connections: List[str] = field(default_factory=list) # list of room_ids
    encounters: List[Encounter] = field(default_factory=list) # list of encounters 
...
class Dungeon:
    ...
    description: str
    difficulty: DifficultyType
    start_room: str # room_id
    end_room: str # room_id
